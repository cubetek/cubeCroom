...
The final job validates the complete asset set, signs a release manifest, uploads and verifies the bytes in the draft, publishes once, and checks every download anonymously. There is no account or token embedded in the app. `downloads.json` is generated beside the signed manifest for documentation consumers. The static docs `/download` page reads GitHub's public latest-release API directly, verifies that it is a published stable release and that all configured installer names, sizes, and official URLs match, then displays those links. Its build-time server component reads the central configuration and passes only JSON to the browser. No Node code or token is bundled into the client. API CORS was checked directly: both a public successful response and CubeCroom's current 404 include `Access-Control-Allow-Origin: *`. With no public release, the page explains that the first release is being prepared and links to the guide; network/validation failures have their own retry state. A documentation-host deployment remains external because no host has been selected.
...
This smoke launches the unpacked executable. It does not validate installer integration, macOS Gatekeeper installation, or the AppImage desktop launcher. The installed electron-builder 26.15.3 source adds `--no-sandbox` to its default AppImage desktop entry; launching `linux-unpacked` does not exercise that entry. Native release acceptance must cover the actual distributed installer/launcher and host sandbox behavior, as well as A-to-B updates.
...
These checks run on the available Windows host. No signing credentials were used, no GitHub workflow was dispatched, and neither macOS/Linux installer execution nor an installed A-to-B update is claimed by these tests.
